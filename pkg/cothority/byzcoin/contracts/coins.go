...
func (c *contractCoin) Spawn(rst byzcoin.ReadOnlyStateTrie, inst byzcoin.Instruction, coins []byzcoin.Coin) (sc []byzcoin.StateChange, cout []byzcoin.Coin, err error) {
	...
	if inst.Spawn.Args.Search("public") != nil {
		ca, err = inst.DeriveIDArg("", "public")
		if err != nil{
			return nil, nil, xerrors.Errorf("while getting DeriveIDArg: %v",
				err)
		}
	} else {
		ca, err = inst.DeriveIDArg("", "coinID")
		if err != nil{
			return nil, nil, xerrors.Errorf("while getting DeriveIDArg: %v",
				err)
		}
